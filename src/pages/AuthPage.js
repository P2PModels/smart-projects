...
const LinkToOtherForm = ({ classes, question, button, onClick }) => (
  <div className={classes.marginTop}>
    <Typography align="center">{question} <Button variant="outlined" color="primary" onClick={onClick}>{button}</Button></Typography>
  </div>
)
...
function Auth({ classes, history,  match: { path } }) {
  ...
  const { title, subtitle, button, isLogin, otherLink } = path !== '/register' ? login : register
  ...
  return (
    <div className={classes.background} style={{backgroundImage: 'url(/images/' + (isLogin ? 'login' : 'register') + '-background.jpg)'}}>
      <main className={classes.main}>
        ...
        <Paper className={classes.paper}>
          ...
          <form className={classes.form} onSubmit={onSubmit}>
            ...
            <LinkToOtherForm classes={classes} {...otherLink} onClick={() => history.push(otherLink.link)} />
          </form>
        </Paper>
      </main>
    </div>
